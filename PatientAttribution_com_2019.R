...
dir_out <- output_dir('E:/CT_APCD/Sai/lds2021/lds2021file/Attribution_result/InProcess_2019/')
## READ MC and NPPES
nppes0 <- fread("E:/CT_APCD/Sai/lds2021/lds2021file/nppes/nppes_lds_com_2019_short.csv",
...
elig.patient[, age:= 2019 - as.integer(birth_year)]
...
# patient_age0064 <- elig.patient[birth_dt >= 2019 - 64]$INTERNAL_MEMBER_ID  # by Sai
...
mcpt <- rbindlist(lapply(paste0("E:/CT_APCD/Sai/lds2021/lds2021file/medicare_medicare_ad_patients/medicare_medicare_ad_patients_", 2018:2019, ".csv"),
                         function(x){
                           fread(x, colClasses = "character")
                         })) %>% unique()
MC_2018_OutP <- MC_2018_OutP[!internal_member_id %in% mcpt$internal_member_id]
MC_2019_OutP <- MC_2019_OutP[!internal_member_id %in% mcpt$internal_member_id]
...
MC_2019_OutP <- elig.patient[MC_2019_OutP, on = c("internal_member_id"),
                             nomatch = 0]
MC_2018_OutP <- elig.patient[MC_2018_OutP, on = c("internal_member_id"),
                             nomatch = 0]

## remove facility provider
# MC_2019_OutP <- MC_2019_OutP[!provider_facility, on = 'PROVIDER_ID']
...
MC_2019_OutP_NonEM <- MC_2019_OutP[!procedure_code %in% CPTList]
...
MC_2019_OutP_EM_pcp <- MC_2019_OutP_EM[NPI %in% realNPI_pcp]
MC_2019_OutP_EM_obgyn <- MC_2019_OutP_EM[NPI %in% realNPI_obgyn]
MC_2018_OutP_EM_pcp <- MC_2018_OutP_EM[NPI %in% realNPI_pcp]
MC_2018_OutP_EM_obgyn <- MC_2018_OutP_EM[NPI %in% realNPI_obgyn]
MC_2019_OutP_EM_other <- MC_2019_OutP_EM[!NPI %in% c(realNPI_pcp, realNPI_obgyn)]


## Patient_Attribution matrix
Patient_classification <- data.table(internal_member_id =
                                       unique(MC_2019_OutP_EM$internal_member_id))
Patient_classification[,
                       `:=` (In_2019_OutP_EM_pcp =
                               as.numeric(internal_member_id %in%
                                            unique(MC_2019_OutP_EM_pcp$internal_member_id)),
                             In_2019_OutP_EM_obgyn =
                               as.numeric(internal_member_id %in%
                                            unique(MC_2019_OutP_EM_obgyn$internal_member_id)),
                             In_2018_OutP_EM_pcp =
                               as.numeric(internal_member_id %in%
...
Patient_classification[In_2019_OutP_EM_pcp == 1,
                       Attribution := 'PCP_2019']
## patient attribution flow chart step2
Patient_classification[In_2019_OutP_EM_pcp == 0 & In_2018_OutP_EM_pcp == 1,
                       Attribution := 'PCP_2018']
## patient attribution flow chart step3
Patient_classification[In_2019_OutP_EM_pcp == 0 & In_2018_OutP_EM_pcp == 0 &
                         In_2019_OutP_EM_obgyn == 1,
                       Attribution := 'OBGYN_2019']
## patient attribution flow chart step4
Patient_classification[In_2019_OutP_EM_pcp == 0 & In_2018_OutP_EM_pcp == 0 &
                         In_2019_OutP_EM_obgyn == 0 & In_2018_OutP_EM_obgyn == 1,
                       Attribution := 'OBGYN_2018']
## patient attribution flow chart step5
Patient_classification[In_2019_OutP_EM_pcp == 0 & In_2018_OutP_EM_pcp == 0 &
                         In_2019_OutP_EM_obgyn == 0 & In_2018_OutP_EM_obgyn == 0,
                       Attribution := 'Unattributed']

Patient_classification[!internal_member_id %in% c(unique(MC_2019_OutP_EM$internal_member_id),
...
patient_to_pcp_2019 <- Patient_classification[Attribution == 'PCP_2019']$internal_member_id
patient_to_pcp_2018 <- Patient_classification[Attribution == 'PCP_2018']$internal_member_id
patient_to_obgyn_2019 <- Patient_classification[Attribution == 'OBGYN_2019']$internal_member_id
...
#### Part 1.1
MC_pcp_2019 <- MC_2019_OutP_EM_pcp[internal_member_id %in% patient_to_pcp_2019]
MC_pcp_2018 <- MC_2018_OutP_EM_pcp[internal_member_id %in% patient_to_pcp_2018]
MC_obgyn_2019 <- MC_2019_OutP_EM_obgyn[internal_member_id %in% patient_to_obgyn_2019]
MC_obgyn_2018 <- MC_2018_OutP_EM_obgyn[internal_member_id %in% patient_to_obgyn_2018]
MC_unattr_2019 <- MC_2019_OutP_EM[internal_member_id %in% patient_unattr]

fwrite(MC_pcp_2019,
       paste0(dir_out, 'MC_pcp_2019.csv'))
fwrite(MC_pcp_2018,
       paste0(dir_out, 'MC_pcp_2018.csv'))
fwrite(MC_obgyn_2019,
       paste0(dir_out, 'MC_obgyn_2019.csv'))
fwrite(MC_obgyn_2018,
       paste0(dir_out, 'MC_obgyn_2018.csv'))
fwrite(MC_unattr_2019,
       paste0(dir_out, 'MC_unattributed_2019.csv'))
...
Patient_PCP_2019_UniqueVisits_EM <- out_unique[, Identify := 'Visits_EM']
Patient_PCP_2019_TieVisits_EM <- out_tie
fwrite(out_unique[, Identify := 'Visits_EM'],
       paste0(dir_out, 'Patient_PCP_2019_UniqueVisits_EM.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_PCP_2019_TieVisits_EM.csv'))
info.summary <- data.table(matrix(c('MC_PCP_2019_Visits_EM',
...
info.summary <- rbind(info.summary,
                      data.table(matrix(c('MC_PCP_2018_Visits_EM',
                                          length(unique(group$internal_member_id)),
                                          dim(out_unique)[1],
                                          dim(out_tie)[1],
                                          length(unique(out_tie$internal_member_id)),
...
Patient_OBGYN_2019_UniqueVisits_EM <- out_unique[, Identify := 'Visits_EM']
fwrite(out_unique[, Identify := 'Visits_EM'],
       paste0(dir_out, 'Patient_OBGYN_2019_UniqueVisits_EM.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_OBGYN_2019_TieVisits_EM.csv'))
info.summary <- rbind(info.summary,
                      data.table(matrix(c('MC_OBGYN_2019_Visits_EM',
                                          length(unique(group$internal_member_id)),
                                          dim(out_unique)[1],
                                          dim(out_tie)[1],
                                          length(unique(out_tie$internal_member_id)),
...
## SS2.1 ##
Patient_PCP_2019_TieVisits_EM <- fread(paste0(dir_out, 'Patient_PCP_2019_TieVisits_EM.csv'),
                                       header = TRUE,
                                       colClasses = 'character',
                                       encoding = 'UTF-8')
MC_PCP_2019_OutP_SS1 <- Patient_PCP_2019_TieVisits_EM[MC_2019_OutP,
                                                      on = c(internal_member_id = 'internal_member_id',
                                                             NPI = 'NPI')][Freq > 0]
group <- MC_PCP_2019_OutP_SS1
{
  tmp1 <- group[,
                list(Freq = length(unique(medical_claim_header_id))),
                by = list(internal_member_id, NPI)]
...
Patient_PCP_2019_UniqueVisits_OutP <- out_unique[, Identify := 'Visits_OutP']
fwrite(out_unique[, Identify := 'Visits_OutP'],
       paste0(dir_out, 'Patient_PCP_2019_UniqueVisits_OutP.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_PCP_2019_TieVisits_OutP.csv'))
tmp <- data.table(matrix(c('MC_PCP_2019_Visits_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS2.3 ##
Patient_OBGYN_2019_TieVisits_EM <- fread(paste0(dir_out, 'Patient_OBGYN_2019_TieVisits_EM.csv'),
                                         header = TRUE,
                                         colClasses = 'character',
                                         encoding = 'UTF-8')
MC_OBGYN_2019_OutP_SS1 <- Patient_OBGYN_2019_TieVisits_EM[MC_2019_OutP,
                                                          on = c(internal_member_id = 'internal_member_id',
                                                                 NPI = 'NPI')][Freq > 0]
group <- MC_OBGYN_2019_OutP_SS1
{
  tmp1 <- group[,
                list(Freq = length(unique(medical_claim_header_id))),
                by = list(internal_member_id, NPI)]
...
Patient_OBGYN_2019_UniqueVisits_OutP <- out_unique[, Identify := 'Visits_OutP']
fwrite(out_unique[, Identify := 'Visits_OutP'],
       paste0(dir_out, 'Patient_OBGYN_2019_UniqueVisits_OutP.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_OBGYN_2019_TieVisits_OutP.csv'))
tmp <- data.table(matrix(c('MC_OBGYN_2019_Visits_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS3.1 ##
Patient_PCP_2019_TieVisits_OutP <- fread(paste0(dir_out, 'Patient_PCP_2019_TieVisits_OutP.csv'),
                                         header = TRUE,
                                         colClasses = 'character',
                                         encoding = 'UTF-8')
MC_PCP_2019_OutP_SS2 <- Patient_PCP_2019_TieVisits_OutP[MC_2019_OutP,
                                                        on = c(internal_member_id = 'internal_member_id',
                                                               NPI = 'NPI')][Freq > 0]
group <- MC_PCP_2019_OutP_SS2
{
  tmp1 <- group[,
...
Patient_PCP_2019_UniqueDays_OutP <- out_unique[, Identify := 'MostDays_OutP']
fwrite(out_unique[, Identify := 'MostDays_OutP'],
       paste0(dir_out, 'Patient_PCP_2019_UniqueDays_OutP.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_PCP_2019_TieDays_OutP.csv'))
tmp <- data.table(matrix(c('MC_PCP_2019_Days_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS3.3 ##
Patient_OBGYN_2019_TieVisits_OutP <- fread(paste0(dir_out, 'Patient_OBGYN_2019_TieVisits_OutP.csv'),
                                           header = TRUE,
                                           colClasses = 'character',
                                           encoding = 'UTF-8')
MC_OBGYN_2019_OutP_SS2 <- Patient_OBGYN_2019_TieVisits_OutP[MC_2019_OutP,
                                                            on = c(internal_member_id = 'internal_member_id',
                                                                   NPI = 'NPI')][Freq > 0]
group <- MC_OBGYN_2019_OutP_SS2
{
  tmp1 <- group[,
                list(Freq = length(unique(first_service_dt))),
                by = list(internal_member_id, NPI)]
...
Patient_OBGYN_2019_UniqueDays_OutP <- out_unique[, Identify := 'MostDays_OutP']
fwrite(out_unique[, Identify := 'MostDays_OutP'],
       paste0(dir_out, 'Patient_OBGYN_2019_UniqueDays_OutP.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_OBGYN_2019_TieDays_OutP.csv'))
tmp <- data.table(matrix(c('MC_OBGYN_2019_Days_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS4.1 ##
Patient_PCP_2019_TieDays_OutP <- fread(paste0(dir_out, 'Patient_PCP_2019_TieDays_OutP.csv'),
                                       header = TRUE,
                                       colClasses = 'character',
                                       encoding = 'UTF-8')
MC_PCP_2019_OutP_SS3 <- Patient_PCP_2019_TieDays_OutP[MC_2019_OutP,
                                                      on = c(internal_member_id = 'internal_member_id',
                                                             NPI = 'NPI')][Freq > 0]
group <- MC_PCP_2019_OutP_SS3
{
  tmp1 <- group[,
...
Patient_PCP_2019_UniqueRecentDate_OutP <- out_unique[, Identify := 'RecentDate_OutP']
fwrite(out_unique[, Identify := 'RecentDate_OutP'],
       paste0(dir_out, 'Patient_PCP_2019_UniqueRecentDate_OutP.csv'))
Patient_PCP_2019_TieRecentDate_OutP <- out_tie
fwrite(out_tie,
       paste0(dir_out, 'Patient_PCP_2019_TieRecentDate_OutP.csv'))
tmp <- data.table(matrix(c('MC_PCP_2019_RecentDate_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS4.3 ##
Patient_OBGYN_2019_TieDays_OutP <- fread(paste0(dir_out, 'Patient_OBGYN_2019_TieDays_OutP.csv'),
                                         header = TRUE,
                                         colClasses = 'character',
                                         encoding = 'UTF-8')
MC_OBGYN_2019_OutP_SS3 <- Patient_OBGYN_2019_TieDays_OutP[MC_2019_OutP,
                                                          on = c(internal_member_id = 'internal_member_id',
                                                                 NPI = 'NPI')][Freq > 0]
group <- MC_OBGYN_2019_OutP_SS3
{
  tmp1 <- group[,
                list(RecentDate = max(ymd(first_service_dt))),
                by = list(internal_member_id, NPI)]
...
Patient_OBGYN_2019_UniqueRecentDate_OutP <- out_unique[, Identify := 'RecentDate_OutP']
fwrite(out_unique[, Identify := 'RecentDate_OutP'],
       paste0(dir_out, 'Patient_OBGYN_2019_UniqueRecentDate_OutP.csv'))
Patient_OBGYN_2019_TieRecentDate_OutP <- out_tie
fwrite(out_tie,
       paste0(dir_out, 'Patient_OBGYN_2019_TieRecentDate_OutP.csv'))
tmp <- data.table(matrix(c('MC_OBGYN_2019_RecentDate_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
Patient_PCP_2019_UniqueRecentDate_OutP <- fread(paste0(dir_out, 'Patient_PCP_2019_UniqueRecentDate_OutP.csv'),
                                                header = TRUE,
                                                colClasses = 'character',
                                                encoding = 'UTF-8')
Patient_PCP_2018_UniqueRecentDate_OutP <- fread(paste0(dir_out, 'Patient_PCP_2018_UniqueRecentDate_OutP.csv'),
                                                ...
                                                colClasses = 'character',
                                                encoding = 'UTF-8')
Patient_OBGYN_2019_UniqueRecentDate_OutP <- fread(paste0(dir_out, 'Patient_OBGYN_2019_UniqueRecentDate_OutP.csv'),
                                                  header = TRUE,
                                                  colClasses = 'character',
                                                  encoding = 'UTF-8')
...
Patient_Unique <- rbind(Patient_PCP_2019_UniqueVisits_EM, Patient_PCP_2019_UniqueVisits_OutP, Patient_PCP_2019_UniqueDays_OutP, Patient_PCP_2019_UniqueRecentDate_OutP,
                        Patient_PCP_2018_UniqueVisits_EM, Patient_PCP_2018_UniqueVisits_OutP, Patient_PCP_2018_UniqueDays_OutP, Patient_PCP_2018_UniqueRecentDate_OutP,
                        Patient_OBGYN_2019_UniqueVisits_EM, Patient_OBGYN_2019_UniqueVisits_OutP, Patient_OBGYN_2019_UniqueDays_OutP, Patient_OBGYN_2019_UniqueRecentDate_OutP,
...
### Ties left
Patient_Tie <- rbind(
  Patient_PCP_2019_TieRecentDate_OutP,
  Patient_PCP_2018_TieRecentDate_OutP,
  Patient_OBGYN_2019_TieRecentDate_OutP,
...
## NPI_freq_unattr <- head(sort(table(MC_unattr_2019$NPI), decreasing = TRUE), n = 50)
...
## NPI_unattr <- data.table(NPI = MC_unattr_2019$NPI)
...
## uniqueNPI_unattr <- data.table(NPI = unique(MC_unattr_2019$NPI))

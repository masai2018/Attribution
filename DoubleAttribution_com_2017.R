...
dir.out <- output_dir('E:/CT_APCD/Sai/lds2021/lds2021file/Attribution_result/output/Double-Attribution_2017/')
dir.in.patAttri <- input_dir('E:/CT_APCD/Sai/lds2021/lds2021file/Attribution_result/InProcess_2017/')
dir.in.NPIAttri <- input_dir("E:/CT_APCD/Sai/output/Attribution/NPI-Attribution/")
dir.demo <- output_dir('E:/CT_APCD/Sai/lds2021/lds2021file/Attribution_result/output/demo_2017/')
...
uniqueN(patAttri$internal_member_id) # 697997
patAttriValid <- patAttri[!Identify %in% c('Unattributed', '')] # '' refers to Tie
uniqueN(patAttriValid$internal_member_id) #649927
uniqueN(patAttri[Identify %in% c('Unattributed')]$internal_member_id) #47390
uniqueN(patAttri[Identify %in% c('')]$internal_member_id) #680
npiAttri <- fread(paste0(dir.in.NPIAttri, 'NPI_WholeSet_0613.csv'),
                  header = TRUE, colClasses = 'character', encoding = 'UTF-8')
if (overlap) {
...
uniqueN(Attri2$internal_member_id)  #697997
Attri2valid <- Attri2[!is.na(Total)]
uniqueN(Attri2valid$internal_member_id) # 400065
...
entityNameList <- names(npiAttri)[6:23]
...
uniqueN(npiAttrivalid$NPI) # 2705 providers are attributed to at least 1 patients
...
Patient_Tie <- fread(paste0(dir.in.patAttri, 'PatientTie.csv'),
                     header = TRUE, colClasses = "character", encoding = 'UTF-8')
NPI_WholeSet <- fread(paste0(dir.in.NPIAttri, 'NPI_WholeSet_0613.csv'),
                      header = TRUE, colClasses = "character", encoding = 'UTF-8')
Patient_Tie <- NPI_WholeSet[Patient_Tie, on = "NPI"]

Patient_Tie2 <- Patient_Tie[
  , c(6:23, # corresponds to AN indicators
      25 # internal_member_id
  )][
    , lapply(.SD, function(x) sum(as.numeric(x), na.rm = T)), by = .(internal_member_id)
    ]

Patient_Tie2[, AN_max := do.call(pmax, .SD), .SDcols = 2:19]
...
Patient_Tie3 <- cbind(
  ...
  Patient_Tie2[!AN_max == 0, ifelse(.SD == AN_max, 1, 0), .SDcols = 2:19]
)
uniqueN(Patient_Tie2$internal_member_id)[1]    # 680   
uniqueN(Patient_Tie3$internal_member_id)   #428 have AN
...
uniqueN(Attri2_toAN$internal_member_id) # 400493
...
uniqueN(Attri2_outAN$internal_member_id) # 250114
...
uniqueN(Attri2_final$internal_member_id) # 650607
...
elig[, age:= 2017 - as.integer(birth_year)]
elig <- elig[age < 65]
...
# nppes <- fread("E:/CT_APCD/Sai/lds2021/lds2021file/nppes/nppes_lds_com_2017_short.csv",
...
### discover those
Attri2valid <- fread(file.path(dir.out, "Attri2valid.csv"), colClasses = 'character')
length(unique(Attri2valid$NPI)) # 2571
npiAttri <- fread(paste0(dir.in.NPIAttri, 'NPI_WholeSet_0613.csv'),
                  header = TRUE, colClasses = 'character', encoding = 'UTF-8')
uniqueN(npiAttri$NPI) # 3539

...
dir.out <- output_dir('E:/CT_APCD/Sai/lds2021/lds2021file/Attribution_result/output/Double-Attribution_2018/')
dir.in.patAttri <- input_dir('E:/CT_APCD/Sai/lds2021/lds2021file/Attribution_result/InProcess_2018/')
dir.in.NPIAttri <- input_dir("E:/CT_APCD/Sai/lds2021/lds2021code/Attribution_2018_2019/NPI-Attribution/")
dir.demo <- output_dir('E:/CT_APCD/Sai/lds2021/lds2021file/Attribution_result/output/demo_2018/')
...
uniqueN(patAttri$internal_member_id) # 677940
patAttriValid <- patAttri[!Identify %in% c('Unattributed', '')] # '' refers to Tie
uniqueN(patAttriValid$internal_member_id) #634209
uniqueN(patAttri[Identify %in% c('Unattributed')]$internal_member_id) #43079
uniqueN(patAttri[Identify %in% c('')]$internal_member_id) #652
npiAttri <- fread(paste0(dir.in.NPIAttri, 'NPI_WholeSet_2018_com.csv'),
                  header = TRUE, colClasses = 'character', encoding = 'UTF-8')
if (overlap) {
...
uniqueN(Attri2$internal_member_id)  #677940
Attri2valid <- Attri2[!is.na(Total)]
uniqueN(Attri2valid$internal_member_id) # 390468
...
uniqueN(npiAttrivalid$NPI) # 2674 providers are attributed to at least 1 patients
...
Patient_Tie <- fread(paste0(dir.in.patAttri, 'PatientTie.csv'),
                     header = TRUE, colClasses = "character", encoding = 'UTF-8')
NPI_WholeSet <- fread(paste0(dir.in.NPIAttri, 'NPI_WholeSet_2018_com.csv'),
                      header = TRUE, colClasses = "character", encoding = 'UTF-8')
...
uniqueN(Patient_Tie3$internal_member_id)   #406 have AN
...
uniqueN(Attri2_toAN$internal_member_id) # 390874
...
uniqueN(Attri2_outAN$internal_member_id) # 243987
...
uniqueN(Attri2_final$internal_member_id) # 634861
...
elig[, age:= 2018 - as.integer(birth_year)]
elig <- elig[age < 65]
...
# nppes <- fread("E:/CT_APCD/Sai/lds2021/lds2021file/nppes/nppes_lds_com_2018_short.csv",
...
##     # )
## )
## MC_2018_OutP <- MC_2018_OutP[, .(internal_member_id, submitter_id, procedure_code, rendering_provider_id)]
...
## patient_age0064 <- elig.patient[birth_dt >= 2018 - 64]$INTERNAL_MEMBER_ID
##
...
## MC_2018_OutP <- MC_2018_OutP[internal_member_id %in% patient_age0064]
## MC_2016_OutP <- MC_2016_OutP[internal_member_id %in% patient_age0064]
##
## ## Exclude Medicare claims
## MC_2018_OutP <- MC_2018_OutP[!submitter_id == 15227]
...
##
## MC_2018_npi.no.patient <- MC_2018_OutP_EM[NPI %in% npi.no.patient]
## dim(MC_2018_npi.no.patient)
##                                         # claims that associate with those NPI who get no patient attributed to: 51659 (dt: 2019-04-14)
## length(unique(MC_2018_npi.no.patient$NPI))
##                                         # npi who associate with at least 1 claim in 2018: 364 (dt: 2019-04-14)
## length(unique(MC_2018_npi.no.patient$internal_member_id))
...
##
## ## 16-17 overlap
## length(unique(c(MC_2018_npi.no.patient$NPI, MC_2016_npi.no.patient)))
## # 370 (dt: 2019-04-14)
## length(unique(c(MC_2018_npi.no.patient$internal_member_id, MC_2016_npi.no.patient$internal_member_id)))
## # 41825 (dt: 2019-04-14)
## patient.target <- unique(MC_2018_npi.no.patient$internal_member_id)

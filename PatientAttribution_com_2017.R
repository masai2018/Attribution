...
dir_out <- output_dir('E:/CT_APCD/Sai/lds2021/lds2021file/Attribution_result/InProcess_2017/')
## READ MC and NPPES
nppes0 <- fread("E:/CT_APCD/Sai/lds2021/lds2021file/nppes/nppes_lds_com_2017_short.csv",
...
col.select <-  c('medical_claim_service_line_id',
                 ...
                 "product_code",
                 NULL)
## Outpatient value set ### coomentted by Sai 05/25/2017
...
elig.patient[, age:= 2017 - as.integer(birth_year)]
...
# patient_age0064 <- elig.patient[birth_dt >= 2017 - 64]$INTERNAL_MEMBER_ID  # by Sai
...
mcpt <- rbindlist(lapply(paste0("E:/CT_APCD/Sai/lds2021/lds2021file/medicare_medicare_ad_patients/medicare_medicare_ad_patients_", 2016:2017, ".csv"),
                         function(x){
                           fread(x, colClasses = "character")
                         })) %>% unique()
MC_2016_OutP <- MC_2016_OutP[!internal_member_id %in% mcpt$internal_member_id]
...
MC_2017_OutP <- elig.patient[MC_2017_OutP, on = c("internal_member_id"),
                             nomatch = 0]
MC_2016_OutP <- elig.patient[MC_2016_OutP, on = c("internal_member_id"),
                             nomatch = 0]

## remove facility provider
# MC_2017_OutP <- MC_2017_OutP[!provider_facility, on = 'PROVIDER_ID']
# MC_2016_OutP <- MC_2016_OutP[!provider_facility, on = 'PROVIDER_ID']
...
MC_2016_OutP_NonEM <- MC_2016_OutP[!procedure_code %in% CPTList]
...
MC_2016_OutP_EM_pcp <- MC_2016_OutP_EM[NPI %in% realNPI_pcp]
MC_2016_OutP_EM_obgyn <- MC_2016_OutP_EM[NPI %in% realNPI_obgyn]
MC_2017_OutP_EM_other <- MC_2017_OutP_EM[!NPI %in% c(realNPI_pcp, realNPI_obgyn)]
...
Patient_classification[,
                       `:=` (In_2017_OutP_EM_pcp =
                             ...
                             In_2016_OutP_EM_pcp =
                               as.numeric(internal_member_id %in%
                                            unique(MC_2016_OutP_EM_pcp$internal_member_id)),
                             In_2016_OutP_EM_obgyn =
                               as.numeric(internal_member_id %in%
                                            unique(MC_2016_OutP_EM_obgyn$internal_member_id)))]
## patient attribution flow chart step1
Patient_classification[In_2017_OutP_EM_pcp == 1,
                       Attribution := 'PCP_2017']
## patient attribution flow chart step2
Patient_classification[In_2017_OutP_EM_pcp == 0 & In_2016_OutP_EM_pcp == 1,
                       Attribution := 'PCP_2016']
## patient attribution flow chart step3
Patient_classification[In_2017_OutP_EM_pcp == 0 & In_2016_OutP_EM_pcp == 0 &
                         In_2017_OutP_EM_obgyn == 1,
                       Attribution := 'OBGYN_2017']
## patient attribution flow chart step4
Patient_classification[In_2017_OutP_EM_pcp == 0 & In_2016_OutP_EM_pcp == 0 &
                         In_2017_OutP_EM_obgyn == 0 & In_2016_OutP_EM_obgyn == 1,
                       Attribution := 'OBGYN_2016']
## patient attribution flow chart step5
Patient_classification[In_2017_OutP_EM_pcp == 0 & In_2016_OutP_EM_pcp == 0 &
                         In_2017_OutP_EM_obgyn == 0 & In_2016_OutP_EM_obgyn == 0,
                       Attribution := 'Unattributed']

Patient_classification[!internal_member_id %in% c(unique(MC_2017_OutP_EM$internal_member_id),
                                                  unique(MC_2016_OutP_EM$internal_member_id)),
...
patient_to_pcp_2016 <- Patient_classification[Attribution == 'PCP_2016']$internal_member_id
patient_to_obgyn_2017 <- Patient_classification[Attribution == 'OBGYN_2017']$internal_member_id
patient_to_obgyn_2016 <- Patient_classification[Attribution == 'OBGYN_2016']$internal_member_id
...
MC_pcp_2016 <- MC_2016_OutP_EM_pcp[internal_member_id %in% patient_to_pcp_2016]
MC_obgyn_2017 <- MC_2017_OutP_EM_obgyn[internal_member_id %in% patient_to_obgyn_2017]
MC_obgyn_2016 <- MC_2016_OutP_EM_obgyn[internal_member_id %in% patient_to_obgyn_2016]
MC_unattr_2017 <- MC_2017_OutP_EM[internal_member_id %in% patient_unattr]

fwrite(MC_pcp_2017,
       paste0(dir_out, 'MC_pcp_2017.csv'))
fwrite(MC_pcp_2016,
       paste0(dir_out, 'MC_pcp_2016.csv'))
fwrite(MC_obgyn_2017,
       paste0(dir_out, 'MC_obgyn_2017.csv'))
fwrite(MC_obgyn_2016,
       paste0(dir_out, 'MC_obgyn_2016.csv'))
fwrite(MC_unattr_2017,
       paste0(dir_out, 'MC_unattributed_2017.csv'))
...
Patient_PCP_2017_TieVisits_EM <- out_tie
...
info.summary <- data.table(matrix(c('MC_PCP_2017_Visits_EM',
...
Patient_PCP_2016_UniqueVisits_EM <- out_unique[, Identify := 'Visits_EM']
fwrite(out_unique[, Identify := 'Visits_EM'],
       paste0(dir_out, 'Patient_PCP_2016_UniqueVisits_EM.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_PCP_2016_TieVisits_EM.csv'))
info.summary <- rbind(info.summary,
                      data.table(matrix(c('MC_PCP_2016_Visits_EM',
                                          length(unique(group$internal_member_id)),
                                          dim(out_unique)[1],
                                          dim(out_tie)[1],
                                          length(unique(out_tie$internal_member_id)),
...
Patient_OBGYN_2016_UniqueVisits_EM <- out_unique[, Identify := 'Visits_EM']
fwrite(out_unique[, Identify := 'Visits_EM'],
       paste0(dir_out, 'Patient_OBGYN_2016_UniqueVisits_EM.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_OBGYN_2016_TieVisits_EM.csv'))
info.summary <- rbind(info.summary,
                      data.table(matrix(c('MC_OBGYN_2016_Visits_EM',
                                          length(unique(group$internal_member_id)),
                                          dim(out_unique)[1],
                                          dim(out_tie)[1],
                                          length(unique(out_tie$internal_member_id)),
...
## SS2.2 ##
Patient_PCP_2016_TieVisits_EM <- fread(paste0(dir_out, 'Patient_PCP_2016_TieVisits_EM.csv'),
                                       header = TRUE,
                                       colClasses = 'character',
                                       encoding = 'UTF-8')
MC_PCP_2016_OutP_SS1 <- Patient_PCP_2016_TieVisits_EM[MC_2016_OutP,
                                                      on = c(internal_member_id = 'internal_member_id',
                                                             NPI = 'NPI')][Freq > 0]
group <- MC_PCP_2016_OutP_SS1
{
  tmp1 <- group[,
                list(Freq = length(unique(medical_claim_header_id))),
                by = list(internal_member_id, NPI)]
...
Patient_PCP_2016_UniqueVisits_OutP <- out_unique[, Identify := 'Visits_OutP']
fwrite(out_unique[, Identify := 'Visits_OutP'],
       paste0(dir_out, 'Patient_PCP_2016_UniqueVisits_OutP.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_PCP_2016_TieVisits_OutP.csv'))
tmp <- data.table(matrix(c('MC_PCP_2016_Visits_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS2.4 ##
Patient_OBGYN_2016_TieVisits_EM <- fread(paste0(dir_out, 'Patient_OBGYN_2016_TieVisits_EM.csv'),
                                         header = TRUE,
                                         colClasses = 'character',
                                         encoding = 'UTF-8')
MC_OBGYN_2016_OutP_SS1 <- Patient_OBGYN_2016_TieVisits_EM[MC_2016_OutP,
                                                          on = c(internal_member_id = 'internal_member_id',
                                                                 NPI = 'NPI')][Freq > 0]
group <- MC_OBGYN_2016_OutP_SS1
{
  tmp1 <- group[,
                list(Freq = length(unique(medical_claim_header_id))),
                by = list(internal_member_id, NPI)]
...
Patient_OBGYN_2016_UniqueVisits_OutP <- out_unique[, Identify := 'Visits_OutP']
fwrite(out_unique[, Identify := 'Visits_OutP'],
       paste0(dir_out, 'Patient_OBGYN_2016_UniqueVisits_OutP.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_OBGYN_2016_TieVisits_OutP.csv'))
tmp <- data.table(matrix(c('MC_OBGYN_2016_Visits_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS3.2 ##
Patient_PCP_2016_TieVisits_OutP <- fread(paste0(dir_out, 'Patient_PCP_2016_TieVisits_OutP.csv'),
                                         header = TRUE,
                                         colClasses = 'character',
                                         encoding = 'UTF-8')
MC_PCP_2016_OutP_SS2 <- Patient_PCP_2016_TieVisits_OutP[MC_2016_OutP,
                                                        on = c(internal_member_id = 'internal_member_id',
                                                               NPI = 'NPI')][Freq > 0]
group <- MC_PCP_2016_OutP_SS2
{
  tmp1 <- group[,
                list(Freq = length(unique(first_service_dt))),
                by = list(internal_member_id, NPI)]
...
Patient_PCP_2016_UniqueDays_OutP <- out_unique[, Identify := 'MostDays_OutP']
fwrite(out_unique[, Identify := 'MostDays_OutP'],
       paste0(dir_out, 'Patient_PCP_2016_UniqueDays_OutP.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_PCP_2016_TieDays_OutP.csv'))
tmp <- data.table(matrix(c('MC_PCP_2016_Days_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS3.4 ##
Patient_OBGYN_2016_TieVisits_OutP <- fread(paste0(dir_out, 'Patient_OBGYN_2016_TieVisits_OutP.csv'),
                                           header = TRUE,
                                           colClasses = 'character',
                                           encoding = 'UTF-8')
MC_OBGYN_2016_OutP_SS2 <- Patient_OBGYN_2016_TieVisits_OutP[MC_2016_OutP,
                                                            on = c(internal_member_id = 'internal_member_id',
                                                                   NPI = 'NPI')][Freq > 0]
group <- MC_OBGYN_2016_OutP_SS2
{
  tmp1 <- group[,
                list(Freq = length(unique(first_service_dt))),
                by = list(internal_member_id, NPI)]
...
Patient_OBGYN_2016_UniqueDays_OutP <- out_unique[, Identify := 'MostDays_OutP']
fwrite(out_unique[, Identify := 'MostDays_OutP'],
       paste0(dir_out, 'Patient_OBGYN_2016_UniqueDays_OutP.csv'))
fwrite(out_tie,
       paste0(dir_out, 'Patient_OBGYN_2016_TieDays_OutP.csv'))
tmp <- data.table(matrix(c('MC_OBGYN_2016_Days_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS4.2 ##
Patient_PCP_2016_TieDays_OutP <- fread(paste0(dir_out, 'Patient_PCP_2016_TieDays_OutP.csv'),
                                       header = TRUE,
                                       colClasses = 'character',
                                       encoding = 'UTF-8')
MC_PCP_2016_OutP_SS3 <- Patient_PCP_2016_TieDays_OutP[MC_2016_OutP,
                                                      on = c(internal_member_id = 'internal_member_id',
                                                             NPI = 'NPI')][Freq > 0]
group <- MC_PCP_2016_OutP_SS3
{
  tmp1 <- group[,
                list(RecentDate = max(ymd(first_service_dt))),
                by = list(internal_member_id, NPI)]
...
Patient_PCP_2016_UniqueRecentDate_OutP <- out_unique[, Identify := 'RecentDate_OutP']
fwrite(out_unique[, Identify := 'RecentDate_OutP'],
       paste0(dir_out, 'Patient_PCP_2016_UniqueRecentDate_OutP.csv'))
Patient_PCP_2016_TieRecentDate_OutP <- out_tie
fwrite(out_tie,
       paste0(dir_out, 'Patient_PCP_2016_TieRecentDate_OutP.csv'))
tmp <- data.table(matrix(c('MC_PCP_2016_RecentDate_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
## SS4.4 ##
Patient_OBGYN_2016_TieDays_OutP <- fread(paste0(dir_out, 'Patient_OBGYN_2016_TieDays_OutP.csv'),
                                         header = TRUE,
                                         colClasses = 'character',
                                         encoding = 'UTF-8')
MC_OBGYN_2016_OutP_SS3 <- Patient_OBGYN_2016_TieDays_OutP[MC_2016_OutP,
                                                          on = c(internal_member_id = 'internal_member_id',
                                                                 NPI = 'NPI')][Freq > 0]
group <- MC_OBGYN_2016_OutP_SS3
{
  tmp1 <- group[,
                list(RecentDate = max(ymd(first_service_dt))),
                by = list(internal_member_id, NPI)]
...
Patient_OBGYN_2016_UniqueRecentDate_OutP <- out_unique[, Identify := 'RecentDate_OutP']
fwrite(out_unique[, Identify := 'RecentDate_OutP'],
       paste0(dir_out, 'Patient_OBGYN_2016_UniqueRecentDate_OutP.csv'))
Patient_OBGYN_2016_TieRecentDate_OutP <- out_tie
fwrite(out_tie,
       paste0(dir_out, 'Patient_OBGYN_2016_TieRecentDate_OutP.csv'))
tmp <- data.table(matrix(c('MC_OBGYN_2016_RecentDate_OutP',
                           length(unique(group$internal_member_id)),
                           dim(out_unique)[1],
                           dim(out_tie)[1],
                           length(unique(out_tie$internal_member_id)),
...
Patient_PCP_2017_UniqueRecentDate_OutP <- fread(paste0(dir_out, 'Patient_PCP_2017_UniqueRecentDate_OutP.csv'),
                                                ...
                                                colClasses = 'character',
                                                encoding = 'UTF-8')
Patient_PCP_2016_UniqueRecentDate_OutP <- fread(paste0(dir_out, 'Patient_PCP_2016_UniqueRecentDate_OutP.csv'),
                                                header = TRUE,
                                                colClasses = 'character',
                                                encoding = 'UTF-8')
Patient_OBGYN_2017_UniqueRecentDate_OutP <- fread(paste0(dir_out, 'Patient_OBGYN_2017_UniqueRecentDate_OutP.csv'),
                                                  ...
                                                  colClasses = 'character',
                                                  encoding = 'UTF-8')
Patient_OBGYN_2016_UniqueRecentDate_OutP <- fread(paste0(dir_out, 'Patient_OBGYN_2016_UniqueRecentDate_OutP.csv'),
                                                  header = TRUE,
                                                  colClasses = 'character',
                                                  encoding = 'UTF-8')

Patient_Unique <- rbind(Patient_PCP_2017_UniqueVisits_EM, Patient_PCP_2017_UniqueVisits_OutP, Patient_PCP_2017_UniqueDays_OutP, Patient_PCP_2017_UniqueRecentDate_OutP,
                        Patient_PCP_2016_UniqueVisits_EM, Patient_PCP_2016_UniqueVisits_OutP, Patient_PCP_2016_UniqueDays_OutP, Patient_PCP_2016_UniqueRecentDate_OutP,
                        ...
                        Patient_OBGYN_2016_UniqueVisits_EM, Patient_OBGYN_2016_UniqueVisits_OutP, Patient_OBGYN_2016_UniqueDays_OutP, Patient_OBGYN_2016_UniqueRecentDate_OutP,
                        fill = TRUE)
...
Patient_Tie <- rbind(
  ...
  Patient_PCP_2016_TieRecentDate_OutP,
  Patient_OBGYN_2017_TieRecentDate_OutP,
  Patient_OBGYN_2016_TieRecentDate_OutP,
  fill = TRUE
)
fwrite(Patient_Tie,
...
# Patient_classification2 <- nppes1[Patient_classification1, on = c(NPI = 'NPI')]
...
## NPI_freq_unattr <- head(sort(table(MC_unattr_2017$NPI), decreasing = TRUE), n = 50)
...
## NPI_unattr <- data.table(NPI = MC_unattr_2017$NPI)
...
## uniqueNPI_unattr <- data.table(NPI = unique(MC_unattr_2017$NPI))

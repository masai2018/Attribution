...
### prerequisite
library(data.table)
## server <- (Sys.info()[4] == 'LAZ-DEID1') ## logical value
...
dir.out <- output_dir('E:/CT_APCD/Sai/Attribution_2018_2019/output/Double-Attribution_mc_2019/')
dir.in.patAttri <- input_dir('E:/CT_APCD/Attribution_2018_2019/InProcess_mc_2019/')
dir.in.NPIAttri <- input_dir("E:/CT_APCD/Sai/Attribution_2018_2019/NPI-Attribution/")
dir.demo <- output_dir('E:/CT_APCD/Sai/Attribution_2018_2019/output/demo_mc_2019/')
...
uniqueN(patAttri$internal_member_id) # 578111
patAttriValid <- patAttri[!Identify %in% c('Unattributed', '')] # '' refers to Tie
uniqueN(patAttriValid$internal_member_id) #540300
npiAttri <- fread(paste0(dir.in.NPIAttri, 'NPI_WholeSet_07162020_medicare.csv'),
                  header = TRUE, colClasses = 'character', encoding = 'UTF-8')
if (overlap) {
...
uniqueN(Attri2$internal_member_id)  #602492
Attri2valid <- Attri2[!is.na(Total)]
uniqueN(Attri2valid$internal_member_id) # 350944
...
entityNameList <- names(npiAttri)[6:39]
...
uniqueN(npiAttrivalid$NPI) # 2591 providers are attributed to at least 1 patients
...
Patient_Tie <- fread(paste0(dir.in.patAttri, 'PatientTie.csv'),
                     header = TRUE, colClasses = "character", encoding = 'UTF-8')
NPI_WholeSet <- fread(paste0(dir.in.NPIAttri, 'NPI_WholeSet_07162020_medicare.csv'),
                      header = TRUE, colClasses = "character", encoding = 'UTF-8')
Patient_Tie <- NPI_WholeSet[Patient_Tie, on = "NPI"]

Patient_Tie2 <- Patient_Tie[
  , c(6:39, # corresponds to AN indicators
      41 # internal_member_id
  )][
    , lapply(.SD, function(x) sum(as.numeric(x), na.rm = T)), by = .(internal_member_id)
    ]

Patient_Tie2[, AN_max := do.call(pmax, .SD), .SDcols = 2:35]
...
Patient_Tie3 <- cbind(
  ...
  Patient_Tie2[!AN_max == 0, ifelse(.SD == AN_max, 1, 0), .SDcols = 2:35]
)
uniqueN(Patient_Tie2$internal_member_id)[1]    # 777   
uniqueN(Patient_Tie3$internal_member_id)   #354 have AN
...
uniqueN(Attri2_toAN$internal_member_id) # 351298
...
uniqueN(Attri2_outAN$internal_member_id) # 196633
...
uniqueN(Attri2_final$internal_member_id) # 541077
elig1 <- fread(paste0('E:/CT_APCD/Beth/data4/',
               ...
               encoding = 'UTF-8', select = c(
                 ...
                 "birth_dt",
                 "GENDER_CODE",
                 NULL)) %>% unique(use.key = FALSE)
elig2 <- fread("E:/CT_APCD/shared/intermediate_data/APCD_modified/medicare_medicare_ad_patients/fy2017all_ages.csv",
               header = TRUE, colClasses = 'character',
               encoding = 'UTF-8')
elig <- elig1[elig2, on = "INTERNAL_MEMBER_ID", nomatch = 0]
sum(!Attri2_final$internal_member_id %in% elig$INTERNAL_MEMBER_ID)
elig[, Age := 2017 - as.integer(birth_dt)]
...
nppes <- fread('E:/CT_APCD/shared/intermediate_data/APCD_modified/provider/nppes_1129.csv',
...
### discover those
Attri2valid <- fread(file.path(dir.out, "Attri2valid.csv"), colClasses = 'character')
length(unique(Attri2valid$NPI)) # 2591
npiAttri <- fread(paste0(dir.in.NPIAttri, 'NPI_WholeSet_07162020_medicare.csv'),
                  header = TRUE, colClasses = 'character', encoding = 'UTF-8')
uniqueN(npiAttri$NPI) # 4017
...
##     # )
## )
## MC_2017_OutP <- MC_2017_OutP[, .(internal_member_id, submitter_id, procedure_code, rendering_provider_id)]
...
## patient_age0064 <- elig.patient[birth_dt >= 2017 - 64]$INTERNAL_MEMBER_ID
##
...
## MC_2017_OutP <- MC_2017_OutP[internal_member_id %in% patient_age0064]
## MC_2016_OutP <- MC_2016_OutP[internal_member_id %in% patient_age0064]
##
## ## Exclude Medicare claims
## MC_2017_OutP <- MC_2017_OutP[!submitter_id == 15227]
...
##
## MC_2017_npi.no.patient <- MC_2017_OutP_EM[NPI %in% npi.no.patient]
## dim(MC_2017_npi.no.patient)
##                                         # claims that associate with those NPI who get no patient attributed to: 51659 (dt: 2019-04-14)
## length(unique(MC_2017_npi.no.patient$NPI))
##                                         # npi who associate with at least 1 claim in 2017: 364 (dt: 2019-04-14)
## length(unique(MC_2017_npi.no.patient$internal_member_id))
...
##
## ## 16-17 overlap
## length(unique(c(MC_2017_npi.no.patient$NPI, MC_2016_npi.no.patient)))
## # 370 (dt: 2019-04-14)
## length(unique(c(MC_2017_npi.no.patient$internal_member_id, MC_2016_npi.no.patient$internal_member_id)))
## # 41825 (dt: 2019-04-14)
## patient.target <- unique(MC_2017_npi.no.patient$internal_member_id)
